...
>
> **v2.1.31**: CC v2.1.218 fork background-default compat — skills count unchanged (44). The 8 `context: fork` producer skills (phase-1-schema, phase-2-convention, phase-3-mockup, phase-4-api, phase-5-design-system, phase-8-review, zero-script-qa, skill-status) each add `background: false` to opt out of CC v2.1.218 background-by-default; qa-phase drops `context: fork` (runs in the main context — AskUserQuestion is stripped at the fork sub-agent boundary, CC #34592/#54892). No SKILL.md count change. CC recommended: v2.1.218.
