...
>
> **v2.1.31**: CC v2.1.218 fork background-default compat — agents unchanged (34, model matrix invariant). Skill-frontmatter-only change (8 producer skills opt out of background-by-default; qa-phase leaves the fork set). CC recommended: v2.1.218.
> **v2.1.30**: Stop-hook stdin-block hardening (#139) — agents unchanged; a runtime-reliability fix in `lib/core/io.js` / `scripts/unified-stop.js` (bounded stdin read so the Stop hook cannot stall on a held-open pipe). No agent definition, model, or count change.
