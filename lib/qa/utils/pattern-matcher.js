...
function extractExports(content) {
  ...
  try {
    ...
    if (meMatch) {
      ...
      if (closePos !== -1) {
        ...
        const startOffset = content.substring(0, meMatch.index).split(/\r?\n/).length;
        ...
        for (const seg of segments) {
          ...
          if (keyMatch) {
            const linesBefore = body.slice(0, seg.start).split(/\r?\n/).length - 1;
  ...
  if (!parsed) {
    ...
    if (moduleExportsMatch) {
      ...
      const startOffset = content.substring(0, moduleExportsMatch.index).split(/\r?\n/).length;
