...
function getJsFiles(globPatterns, excludePatterns = [], rootDir = process.cwd()) {
  ...
  for (const pattern of globPatterns) {
    ...
    // Cross-platform (ENH-329): glob patterns are a forward-slash convention
    // (NOT a filesystem path), so split('/') is correct on all OSes. Author-
    // supplied patterns always use '/'; path.sep is intentionally NOT used here.
