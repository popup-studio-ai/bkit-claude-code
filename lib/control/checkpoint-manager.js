 ...
 *
 * Storage layout:
 *   .bkit/checkpoints/index.json        — checkpoint index
 *   .bkit/checkpoints/cp-{timestamp}.json — individual checkpoint snapshots
 *
...
// v2.1.8 fix B3: use STATE_PATHS registry instead of process.cwd()
const { STATE_PATHS } = require('../core/paths');
// H1 fix (audit): atomic tmp+rename writes via the canonical state-store so a
// mid-write SIGKILL can't truncate the checkpoint index / pdca-status / snapshots.
...
const CHECKPOINTS_DIR = '.bkit/checkpoints';
...
function saveIndex(index) {
  const dir = ensureDir();
  const indexPath = path.join(dir, INDEX_FILE);
  // M5 fix (audit): atomic + the caller's RMW should be wrapped in lockedUpdate;
  // write() here is the atomic backend (tmp+rename) that lockedUpdate uses too.
  stateStore.write(indexPath, index);
}
...
function createCheckpoint(feature, phase, type, description, fileHashes = null) {
  ...
  stateStore.write(cpPath, checkpoint);

  // Update index — M5 fix (audit): locked RMW so concurrent checkpoints can't
  // orphan index entries (snapshot written to disk but never listed/pruned because
  // the loser's push was overwritten). lockedUpdate holds the lock across
  // load→push→save (which itself writes atomically via stateStore.write).
  const indexEntry = {
    id,
  ...
  const indexPath = path.join(ensureDir(), INDEX_FILE);
  stateStore.lockedUpdate(indexPath, (raw) => {
...
function pruneCheckpoints(options = {}) {
  ...
  // (otherEntries removed: computed but never used.)
...
function deleteCheckpoint(checkpointId) {
  const dir = ensureDir(); // v2.1.0: fixed from undefined getCheckpointDir()
  const cpPath = path.join(dir, `${checkpointId}.json`);
 ...
 *
 * v2.1.12 Sprint B-2 (defect #12 fix): the previous implementation hashed
 * the full checkpoint object (without hash fields) and compared it to
 * `pdcaStatusHash`, which `createCheckpoint` derived from ONLY the
 * `pdcaStatus` snapshot. The two hashes were therefore guaranteed to
 * disagree, causing every freshly-created checkpoint to verify as
 * `valid:false` (rollback integrity broken — bare `getCheckpoint(id)` →
 * `verifyCheckpoint(id)` always failed).
 *
 * Fixed semantics: `pdcaStatusHash` represents the integrity of the
 * `pdcaStatus` snapshot. We re-hash that exact field and compare. The
 * legacy `cp.hash` field (full-object hash) is kept for forward
 * compatibility but never produced today.
 *
 * @param {string} checkpointId - Checkpoint ID to verify
 * @returns {{ valid: boolean, expected?: string, actual?: string, hashType?: string }}
 */
function verifyCheckpoint(checkpointId) {
  const cp = getCheckpoint(checkpointId);
  if (!cp) return { valid: false };

  // v2.1.12 #12 fix: prefer pdcaStatusHash (the canonical createCheckpoint
  // output). Fallback to legacy `hash` only if the new field is absent
  // AND a legacy `hash` is present.
  if (cp.pdcaStatusHash != null && cp.pdcaStatus !== undefined) {
    const actual = sha256(JSON.stringify(cp.pdcaStatus));
    return {
      valid: actual === cp.pdcaStatusHash,
      expected: cp.pdcaStatusHash,
      actual,
      hashType: 'pdcaStatusHash',
    };
  }

  if (cp.hash) {
    // Legacy full-object hash path (kept for back-compat). Recompute over
    // the same shape (everything except `hash`).
    const data = { ...cp };
    delete data.hash;
  ...
  // No hash stored, treat as valid (no integrity claim to verify against).
  return { valid: true, hashType: 'none' };
}
