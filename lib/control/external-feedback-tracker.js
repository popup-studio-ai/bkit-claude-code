...
function trackIssues(opts) {
  ...
  for (const handle of dogfooders) {
    try {
      const cmd = `gh issue list --repo ${owner}/${repo} --state all --search "author:${handle} created:>=${sinceDateOnly}" --limit 100 --json number,createdAt,closedAt,author,title`;
      const json = execSync(cmd, { encoding: 'utf8', timeout: 20000, stdio: ['ignore', 'pipe', 'pipe'] });
