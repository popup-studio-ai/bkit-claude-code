...
function resetScore(initialScore = 50, reason = 'manual_reset') {
  ...
  // Fix: profile uses levelHistory, not events (which doesn't exist in schema)
...
/**
 * Sync trust score and level to control-state.json (single direction: trust → control)
 * Called after recordEvent() to keep runtime state in sync.
 */
function syncToControlState() {
  try {
    const automation = require('./automation-controller');
    const profile = loadTrustProfile();
    automation.updateRuntimeState({
      trustScore: profile.trustScore,
    });
  } catch (_) {
    // Non-critical: automation-controller may not be available
  }
}
