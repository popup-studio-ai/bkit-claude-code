 ...
 * v1.4.2: Claude Code API 스키마 완전 준수 (ISSUE-003, ISSUE-006 수정)
...
function outputAllow(context = '', hookEvent = 'PostToolUse') {
  ...
  if (isGeminiCli()) {
    ...
    // v1.4.2: Hook event별 Claude Code API 스키마 준수
    // - PreToolUse: hookEventName + additionalContext (decision 제거, GitHub Issue #16598 회피)
    // - UserPromptSubmit: hookEventName + additionalContext (ISSUE-003: decision 필드 미지원)
    ...
    if (hookEvent === 'Stop') {
      ...
      // ISSUE-003 수정: UserPromptSubmit은 decision 필드 미지원
      // ISSUE-006 수정: hookEventName 필드 추가
      console.log(JSON.stringify({
        hookSpecificOutput: {
 ...
 * @param {boolean} options.skipTask - Force skip Task creation
...
function autoCreatePdcaTask(feature, phase, options = {}) {
  ...
  const subject = generatePdcaTaskSubject(phase, feature);
  const description = generatePdcaTaskDescription(phase, feature, options.docPath);
  const metadata = getPdcaTaskMetadata(phase, feature, options);
  const phaseInfo = PDCA_PHASES[phase.toLowerCase()] || PDCA_PHASES.do;

  // blockedBy is only auto-set for Major Feature
  let blockedBy = [];
  ...
  const taskGuidance = {
    ...
    description,
    metadata,
    blockedBy,  // Empty array or previous phase
    activeForm: `${phaseInfo.emoji || ''} ${phase} phase for ${feature}`
  };
...
module.exports = {
  ...
  // Task System Integration (v1.3.1 - FR-01~05, v1.4.2 - FR-06)
  PDCA_PHASES,
  getPdcaTaskMetadata,
