...
const ACTION_TYPES = [
  ...
  // v2.1.19 S4 (F4-2) — external dogfooder feedback measurement refresh.
  // Emitted by scripts/_v2119-s4-feedback-refresh.js (or future automation)
  // when GitHub API query updates the .bkit/state/external-feedback-tracker.json.
  // Powers Trust Score 7th component externalDogfoodFeedbackResponseRate.
  // details schema: {
  //   dogfooders (string[]), windowDays (number), value (number 0-100|null),
  //   raw: { closed, within24h, openInWindow, issues },
  //   apiErrors (array|null), schemaVersion (string)
  // }
  'external_feedback_tracked',
  // v2.1.19 S3 (F3-1, closes #103) — gate-fail report resolved on successful
  // subsequent transition. Emitted by lib/application/sprint-lifecycle/
  // resolve-gate-fail.js when advancePhase succeeds after a prior gate_fail
  // (sprint.lastGateFailure populated). Idempotent — second call detects
  // existing resolvedAt and emits no-op.
  // details schema: {
  //   sprintId, reportPath (string|null), resolvedAt (ISO), resolvedBy (string),
  //   resolutionReason (string), prependedHeader (boolean — file mutation result)
  // }
  'gate_fail_resolved',
  // v2.1.19 S3 (F3-2, closes #104) — sprint init context auto-imported from
  // master-plan or PRD. Emitted by sprint-handler.handleInit when the
  // context-importer fallback chain succeeds (master-plan first, PRD fallback).
  // details schema: {
  //   sprintId, source ('master-plan'|'prd'|'default'),
  //   filePath (string|null), populatedFields (string[] — WHY/WHO/RISK/SUCCESS/SCOPE)
  // }
  'sprint_context_imported',
  // v2.1.19 S3 (F3-3, closes #105) — divergence detected between sprint.kpi
  // legacy field and sprint.qualityGates SoT values. Emitted by
  // generateReport / status when resolveKpi finds inconsistency.
  // details schema: {
  //   sprintId, divergences: [{ field, kpi, qualityGates }],
  //   precedenceApplied ('qualityGates' — wins per ADR S3-004)
  // }
  'sprint_kpi_divergence',
];
...
/**
 * Resolve the project root that anchors the audit directory.
 *
 * v2.1.26 (I-12, test isolation): an explicitly injected root wins; the
 * default remains `getPlatform().PROJECT_DIR` so runtime behavior without
 * injection is byte-for-byte unchanged.
 *
 * @param {string} [projectRoot] - Optional injected project root
 * @returns {string} Absolute project root
 */
function resolveAuditRoot(projectRoot) {
  if (typeof projectRoot === 'string' && projectRoot.length > 0) return projectRoot;
  return getPlatform().PROJECT_DIR;
}

/**
 * Get the audit directory path
 * @param {string} [projectRoot] - Optional injected project root (v2.1.26 I-12)
 * @returns {string} Absolute path to .bkit/audit/
 */
function getAuditDir(projectRoot) {
  return path.join(resolveAuditRoot(projectRoot), '.bkit', 'audit');
}
 ...
 */
function getAuditFilePath(date, projectRoot) {
  const d = date instanceof Date ? date : (date ? new Date(date) : new Date());
  const dateStr = d.toISOString().slice(0, 10); // YYYY-MM-DD
  return path.join(getAuditDir(projectRoot), `${dateStr}.jsonl`);
}
 ...
 * @param {string} [projectRoot] - Optional injected project root (v2.1.26 I-12)
 */
function ensureAuditDir(projectRoot) {
  const dir = getAuditDir(projectRoot);
 ...
 *
 * v2.1.26 (I-12, test isolation): optional `opts.projectRoot` redirects the
 *   audit dir to `<projectRoot>/.bkit/audit`. When omitted (all existing
 *   callers), resolution is unchanged (`getPlatform().PROJECT_DIR`).
 *
 * @param {Object} entry - Partial or complete AuditLogEntry
 * @param {{ projectRoot?: string }} [opts] - Optional injection context
 */
function writeAuditLog(entry, opts = {}) {
  try {
    const projectRoot = (opts && typeof opts.projectRoot === 'string' && opts.projectRoot.length > 0)
      ? opts.projectRoot
      : undefined;
    ensureAuditDir(projectRoot);
    const normalized = normalizeEntry(entry);
    const line = JSON.stringify(normalized) + '\n';
    const filePath = getAuditFilePath(new Date(normalized.timestamp), projectRoot);
    ...
    if (otelSink && typeof otelSink.emit === 'function') {
      // M6 fix (audit): an async IIFE catches BOTH a synchronous throw from
      // otelSink.emit (which Promise.resolve(syncThrow) does NOT catch — it
      // escaped the old .catch and propagated up the hot writeAuditLog path,
      // affecting hook exit semantics) and an async rejection. Stays fire-and-
      // forget (not awaited here) so the observability-only sink never blocks
      // or breaks the audit write.
      (async () => {
        try {
          await otelSink.emit({
          ...
          /* fire-and-forget — observability-only sink */
        }
      })();
    }
  } catch (e) {
 ...
 * @param {string} [options.projectRoot] - Optional injected project root (v2.1.26 I-12)
 * @returns {Object[]} Array of AuditLogEntry objects
 */
function readAuditLogs(options = {}) {
  try {
    const filePath = getAuditFilePath(options.date, options.projectRoot);
