...
const { computeArchiveReadiness, GATE_DEFINITIONS } = require('../sprint-lifecycle/quality-gates');
 ...
 *
 * Gates outside the 11 explicitly routed return
 * `{ ok: false, reason: 'unsupported_gate' }`. As of Slice 3 Task 3.3, all
 * gates that appear in ACTIVE_GATES_BY_PHASE are routed — UNSUPPORTED_GATES
 * is empty and the "no gate in limbo" guarantee is complete.
 *
...
const GATE_MEASUREMENT_ROUTES = Object.freeze({
  ...
  // Sum of phaseHistory durations converted to hours. Lower is better (count
  // shape). advance-phase.usecase appendExitToHistory stores `durationMs`
  // (ms), so the compute reads durationMs and divides by 3_600_000 — NOT a
  // `durationHours` field (which the phase-history writer never produces).
  M10: Object.freeze({
    agent: null,
    field: 'M10_pdcaCycleTimeHours',
    valueShape: 'count',
    sourceArtifact: 'Sum of sprint.phaseHistory durationMs, converted to hours',
    gateType: 'computed',
    compute: (sprint) => {
      ...
      for (const entry of history) {
        if (entry && typeof entry.durationMs === 'number' && Number.isFinite(entry.durationMs)) {
          sumMs += entry.durationMs;
        }
      }
      return sumMs / 3_600_000;
    },
  }),
  ...
  // Slice 3 Task 3.3: S2 featureCompletion — computed gate, last entry moved
  // out of UNSUPPORTED_GATES. Ratio (0-100) of features whose featureMap
  // completion >= the S2 threshold (default 100). Reads sprint.featureMap
  // safely; an empty featureMap yields 0 (nothing to complete = nothing done;
  // vacuous-truth "100%" would incorrectly green-light an empty sprint). No
  // agent. Caller persists { current: <number>, threshold, passed }.
  S2: Object.freeze({
    agent: null,
    field: 'S2_featureCompletion',
    valueShape: 'percent',
    sourceArtifact: 'Computed: ratio of featureMap entries with completion >= S2 threshold',
    gateType: 'computed',
    compute: (sprint) => {
      const featureMap = (sprint && sprint.featureMap && typeof sprint.featureMap === 'object')
        ? sprint.featureMap
        : {};
      const keys = Object.keys(featureMap);
      const total = keys.length;
      if (total === 0) return 0; // no features = nothing complete (honest 0, not vacuous 100)
      const threshold = (GATE_DEFINITIONS && GATE_DEFINITIONS.S2 && GATE_DEFINITIONS.S2.defaultThreshold !== undefined)
        ? GATE_DEFINITIONS.S2.defaultThreshold
        : 100;
      let done = 0;
      for (const k of keys) {
        const entry = featureMap[k] || {};
        if (typeof entry.completion === 'number' && entry.completion >= threshold) done++;
      }
...
/**
 * Gates the router does NOT yet support. Kept as a frozen array for API
 * stability; emptied in Slice 3 Task 3.3 when S2 became a computed gate.
 * The "no gate in limbo" guarantee: every gate in ACTIVE_GATES_BY_PHASE is
 * now routed in GATE_MEASUREMENT_ROUTES.
 *
 * History of promotions:
 *   - M5 (Slice 2): qa-monitor probe, exemptible via deps.logSourceAvailable=false.
 *   - M10 (Slice 2): computed (phaseHistory duration sum).
 *   - S4 (Slice 2 Task 2.6): computed (all measurable report-phase gates passed AND report exists).
 *   - S2 (Slice 3 Task 3.3): computed (featureMap completion ratio).
 */
