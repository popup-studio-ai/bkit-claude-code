 ...
 *
 * Gates outside the 7 explicitly routed (M5, M10, S2, S4) return
 ...
 * @property {('percent'|'count')} valueShape
...
/** Gates the router does NOT yet support — carried to v2.1.17. */
const UNSUPPORTED_GATES = Object.freeze([
  'M5',  // runtimeErrorRate — needs qa-monitor live docker log probe (carry)
  'M10', // pdcaCycleTimeHours — computed (phaseHistory durations) (carry)
  'S2',  // featureCompletion — computed (kpi.featureCompletionRate) (carry)
  'S4',  // archiveReadiness — boolean preflight (carry)
]);
...
function buildPrompt(gateKey, sprint) {
  ...
  return [
    ...
    '',
    'Source artifact: ' + route.sourceArtifact,
    '',
 ...
 * @property {string} [reason]      - When ok=false: 'unsupported_gate', 'no_agent_runner', 'no_output', 'no_json', 'json_invalid', 'invalid_value', 'agent_error'
