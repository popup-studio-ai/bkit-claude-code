...
async function measureGate(sprint, gateKey, deps) {
  ...
  // 1) Dispatch via router (or test override). Forward logSourceAvailable so
  //    exemptible gates (M5) receive the exemption signal end-to-end: when
  //    the caller signals no runtime logs, the router returns not_applicable
  //    instead of dispatching a qa-monitor probe.
  ...
  const rawResult = await routerImpl.measureGate(gateKey, sprint, {
    ...
    logSourceAvailable: d.logSourceAvailable,
  });
  if (!rawResult.ok) {
    return {
      ok: false,
  ...
  // Slice 3 Task 3.3: when the router asserts its own pass/fail via an
  // explicit `passed` boolean on rawResult, honor it directly and SKIP the
  // evaluateGate re-evaluation. This covers the M5 not_applicable exemption
  // (router returns { value: null, reason: 'not_applicable', passed: true }
  // when deps.logSourceAvailable=false): evaluateGate would otherwise see
  // current:null and flip the exemption to a not_measured failure. Normal
  // agent-measured gates do not carry `passed`, so they fall through to the
  // existing evaluateGate probe unchanged.
  const passed = (typeof rawResult.passed === 'boolean')
    ? rawResult.passed
    : evaluateGate(probeSprint, gateKey).passed;
