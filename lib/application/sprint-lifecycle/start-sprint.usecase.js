...
async function startSprint(input, deps) {
  ...
  while (sprint.phase !== scope.stopAfter && loopCount < HARD_LOOP_LIMIT) {
    ...
    // NOTE (Slice 4, Task 4.3 follow-up): this passes `sprint.phase` (string),
    // so the autorun loop advances phase-by-phase and does NOT exercise the
    // sprint-aware do->qa skip-iterate branch of computeNextPhase. That branch
    // is currently CAPABLE but INERT in production. Wiring it here
    // (computeNextPhase(sprint)) is a one-line change that transitions.js
    // already permits (do->qa is a legal edge), but it expands this loop's
    // behavior into E2E autorun territory (pause-trigger / budget interactions)
    // and is tracked as a separate follow-up task rather than smuggled into 4.3.
