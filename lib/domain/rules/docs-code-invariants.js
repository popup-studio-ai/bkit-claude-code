...
/**
 * GOVERNANCE LOCK (v2.1.22 S4 ENH-336 — do NOT delete these 6 stubs).
 *
 * These 6 `pdca-eval-*` agents are DEPRECATED (v2.1.13) but their stub .md
 * files MUST be retained permanently as deprecation tombstones:
 *   1. Both immutable contract baselines (test/contract/baseline/v2.1.9 AND
 *      v2.1.16 /_MANIFEST.json) list all 6 — historical snapshots, never edited.
 *   2. contract-test-run.js runL4Deprecation() FAILS ("agent '<name>' missing
 *      from current without deprecatedIn declaration") if a baselined agent is
 *      absent from agents/ without a stub carrying `deprecatedIn` frontmatter.
 *   3. The stubs hold `deprecatedIn: v2.1.13` → they currently satisfy L4.
 * Deleting them would (a) break L4 on both baselines and (b) require mutating
 * the immutable historical baselines — both forbidden. S4 (v2.1.22) evaluated
 * removal and CONFIRMED RETENTION. Invariant: Active + Deprecated === agents + 6.
 * See docs/02-design/features/tech-debt-deadcode-elimination.design.md §3.
 */
/** @type {ReadonlyArray<string>} */
