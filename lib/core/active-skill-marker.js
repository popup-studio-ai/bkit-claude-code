...
function markerPath(projectRoot) {
  // v2.1.26 (I-11, test isolation): an explicitly injected root wins; the
 ...
 * @param {{ projectRoot?: string }} [opts] - v2.1.26 (I-11): optional injected
 *   project root; default resolution unchanged.
 */
function writeActiveSkill(entry, opts = {}) {
  if (!entry || !entry.skill) return;
  try {
    const p = markerPath(opts && opts.projectRoot);
