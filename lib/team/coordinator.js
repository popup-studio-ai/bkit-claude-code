...
/**
 * ENH-143: Sequential agent spawn with delay — workaround for #37520.
 *
 * GitHub issue #37520: Parallel agent spawning triggers OAuth 401 errors
 * due to token refresh race conditions. This function spawns agents
 * sequentially with a configurable delay between each spawn to avoid
 * the race condition.
 *
 * When #37520 is resolved upstream, this can be replaced with parallel
 * Promise.all() spawning for better performance.
 *
 * @param {Function} spawnFn - Async function that spawns a single agent.
 *   Signature: (agentDef) => Promise<result>
 * @param {Array<Object>} agents - Array of agent definitions to spawn.
 * @param {Object} [options]
 * @param {number} [options.spawnDelay=2000] - Delay in ms between spawns.
 * @param {number} [options.maxRetries=2] - Max retry attempts on 401 errors.
 * @param {number} [options.retryDelay=3000] - Delay in ms before retry.
 * @returns {Promise<Array<{ agent: Object, result: any, error: Error|null }>>}
 */
async function spawnAgentsSequentially(spawnFn, agents, options = {}) {
  const spawnDelay = options.spawnDelay || 2000;
  ...
  for (let i = 0; i < agents.length; i++) {
    ...
    // Retry loop for 401 errors (#37520 workaround)
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        result = await spawnFn(agent);
        lastError = null;
        break;
      } catch (err) {
        lastError = err;
        const is401 = err.status === 401 || err.code === 401 ||
          (err.message && err.message.includes('401'));

        if (is401 && attempt < maxRetries) {
          // Retry with increasing delay for OAuth race condition
          const delay = retryDelay * (attempt + 1);
          await new Promise(resolve => setTimeout(resolve, delay));
        } else {
          // Non-401 error or exhausted retries — do not retry
          break;
        }
      }
    }

    results.push({ agent, result, error: lastError });

    // Delay between spawns (skip after the last agent)
