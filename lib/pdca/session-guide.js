...
function analyzeModules(designContent) {
  ...
  if (moduleMapMatch) {
    const rows = moduleMapMatch[1].split(/\r?\n/).filter(r => r.includes('|') && !r.match(/^\s*\|[-\s|]+\|/));
...
/**
 * Extract Success Criteria from Plan document as structured list.
 * @param {string} planContent - Full Plan document content
...
function extractSuccessCriteria(planContent) {
  ...
  // Extract from "4. Success Criteria" section
  const sectionMatch = planContent.match(
    /## 4\. Success Criteria[\s\S]*?(?=\n## \d|\n---\n## |$)/
  );
  if (!sectionMatch) return criteria;

  const section = sectionMatch[0];

  // Parse Definition of Done items
  const dodItems = section.match(/Definition of Done[\s\S]*?((?:- \[.\]\s+.+\n?)+)/);
  if (dodItems) {
    const items = dodItems[1].match(/- \[.\]\s+(.+)/g);
    if (items) {
      items.forEach((item, i) => {
        criteria.push({
          id: `SC-${i + 1}`,
  ...
  // Parse Quality Criteria items
  const qualityItems = section.match(/Quality Criteria[\s\S]*?((?:- \[.\]\s+.+\n?)+)/);
  if (qualityItems) {
    const items = qualityItems[1].match(/- \[.\]\s+(.+)/g);
    if (items) {
      items.forEach((item, i) => {
        criteria.push({
          id: `QC-${i + 1}`,
...
/**
 * Format Success Criteria as a tracking table.
 * @param {Array<{ id: string, text: string, type: string }>} criteria
 * @param {Object} [status] - Optional status map { 'SC-1': 'met'|'partial'|'not_met' }
 * @returns {string} Markdown table
 */
function formatSuccessCriteria(criteria, status = {}) {
  if (criteria.length === 0) return '> No Success Criteria found in Plan document.';

  const statusIcon = { met: '✅', partial: '⚠️', not_met: '❌' };
  const lines = [
    '### Success Criteria Tracking',
    '',
    '| ID | Type | Criteria | Status |',
    '|----|------|----------|:------:|',
  ];

  for (const c of criteria) {
    const icon = status[c.id] ? statusIcon[status[c.id]] : '☐';
    lines.push(`| ${c.id} | ${c.type} | ${c.text} | ${icon} |`);
  }

  const met = Object.values(status).filter(s => s === 'met').length;
  lines.push('');
  lines.push(`**Progress**: ${met}/${criteria.length} criteria met`);
