...
/**
 * Resolve the project root for batch state storage.
 *
 * v2.1.26 (I-10, test isolation): an explicitly injected `projectRoot` wins;
 * the default remains `getCore().PROJECT_DIR`, so runtime behavior without
 ...
 * @private
 */
function _resolveProjectRoot(projectRoot) {
 ...
 * @param {string} [projectRoot] - Optional injected project root (v2.1.26 I-10)
 * @returns {string}
 * @private
 */
function _getBatchStatePath(batchId, projectRoot) {
  return path.join(_resolveProjectRoot(projectRoot), '.bkit', 'state', BATCH_STATE_DIR, `${batchId}.json`);
}
 ...
 * @private
 */
function _saveBatchState(batchId, state, projectRoot) {
  const { stateStore } = getCore();
  const statePath = _getBatchStatePath(batchId, projectRoot);
  try {
    state.updatedAt = new Date().toISOString();
    // H1 fix (audit): atomic write of batch state.
    stateStore.write(statePath, state);
 ...
 * @private
 */
function _loadBatchState(batchId, projectRoot) {
  const statePath = _getBatchStatePath(batchId, projectRoot);
  try {
 ...
 * @param {Object} [options] - {workflow, parallel, groupSize, projectRoot}
 *   `projectRoot` (v2.1.26 I-10): optional injected root for batch state
 *   storage; default = getCore().PROJECT_DIR (unchanged runtime behavior).
...
function createBatchPlan(features, options = {}) {
  ...
  const { groupSize = DEFAULT_GROUP_SIZE, projectRoot } = options;
  ...
  // Persist plan
  _saveBatchState(batchId, plan, projectRoot);
 ...
 * @param {Object} [options] - {projectRoot} (v2.1.26 I-10) — optional injected
 *   root for batch state storage; default = getCore().PROJECT_DIR.
 * @returns {{results: Object[], summary: string}}
 */
function executeBatchPlan(plan, options = {}) {
  const { debugLog } = getCore();
  ...
  plan.status = 'running';
  _saveBatchState(plan.batchId, plan, projectRoot);
 ...
 * @param {Object} [options] - {projectRoot} (v2.1.26 I-10)
 * @returns {{status: string, progress: number, features: Object[]}|null}
 */
function getBatchStatus(batchId, options = {}) {
  const state = _loadBatchState(batchId, options.projectRoot);
 ...
 */
function cancelBatch(batchId, options = {}) {
  const { debugLog } = getCore();
  const { projectRoot } = options;
  const state = _loadBatchState(batchId, projectRoot);
  ...
  _saveBatchState(batchId, state, projectRoot);
 ...
 */
function resumeBatch(batchId, options = {}) {
  const { debugLog } = getCore();
  const { projectRoot } = options;
  const state = _loadBatchState(batchId, projectRoot);
