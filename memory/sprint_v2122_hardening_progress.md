...
**✅ 현재 위치 (2026-06-02 세션7 종료 — 회귀 0 확정)**: 7/7 archived + **v2.1.22 순net 회귀 = 0 확정**. 전체 baseline 대조 완료:
- **방법**: baseline(b591410) git worktree(/tmp/bkit-baseline)에서 전체 320 test 파일 실행 → **FAIL 28**. 현재 HEAD 회귀수정 후 전체 321 → **FAIL 26**. `comm -23 current baseline` = **0(current-only 없음)**, `comm -13` = 2(`docs-code-sync`,`sprint-alpha-e2e`는 v2.1.22가 오히려 **고침**). 즉 현재 26 fail ⊊ baseline 28 fail.
- **내 회귀 6건 처리**: ①guard-registry ②hook-wiring ③v200-wiring ④sprint-trust-normalization ⑤pdca-lifecycle(LC-16: state-transitions.js 추출 시 `require('fs')`/`require('path')` 누락→silent action drop) **5건 수정 commit 795c724**. ⑥hook-behavioral-stop STOP-01 = **pre-existing 확정**(baseline full-suite에서도 FAIL, 내 회귀 아님 — active-skill.json 마커 순차오염, Task#8).
- **남은 26 fail = 전부 pre-existing test-debt** → Task#8(v2.1.23 cleanup), v2.1.22 회귀 아님.
- **교훈(불변)**: 회귀 검증은 반드시 (1)전체 test/+tests/ 321 (tests/ 43만으론 test/ 278 회귀 누락), (2)baseline도 **동일 full-suite 순차실행**으로 비교(file-by-file은 순차오염 누락). 추가 함정: `while read` 루프 내 `node "$f"`는 `</dev/null` 없으면 find 스트림 stdin 소비→조기종료(101개에서 멈춤). zsh엔 mapfile 없음.
**[이전 잘못된 기록]** ↓ S5 완료 당시 "릴리스 게이트 충족" 선언(회귀 6건 미검출 상태):
🎉 **7/7 archived — v2.1.22 Hardening Release 완료, 릴리스 게이트 §11 충족** (2026-06-02 세션6). S1·S2·S6·S4·S3a·S3b·S5 전부 archived. version 2.1.22. 남은 것: (1) release/v2.1.22-hardening → main PR/merge(사용자 결정), (2) Task#8 pre-existing test-debt 7건(v2.1.23 carry, v2.1.22 회귀 아님), (3) Windows 실런타임 CI matrix(S2 carry). 
