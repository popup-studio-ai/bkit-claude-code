...
const TMP_ROOT = fs.mkdtempSync(path.join(os.tmpdir(), 'bkit-f1-5-'));
const SPRINTS_DIR = path.join(TMP_ROOT, '.bkit/state/sprints');
...
function cleanupSprint(id) {
  const f = path.join(SPRINTS_DIR, id + '.json');
...
test('TC-F1-5-U1: handleAnnotate on archived sprint → annotation appended + phase preserved', () => {
  ...
  const statePath = path.join(SPRINTS_DIR, 'test-f1-5-archive.json');
...
test('TC-F1-5-CLI: full CLI invocation appends + audit emit', () => {
  ...
  fs.writeFileSync(
    path.join(SPRINTS_DIR, 'test-f1-5-cli.json'),
    JSON.stringify({
